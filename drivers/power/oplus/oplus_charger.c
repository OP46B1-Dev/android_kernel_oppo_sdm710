		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CURRENT_NOW:
			...
			/*
			 * The OPLUS gauge uses negative current while charging.
			 * Match the native QCOM power_supply polarity and
			 * microamp unit.
			 */
		...
		case POWER_SUPPLY_PROP_CHARGE_FULL:
			/* OPLUS reports mAh; power_supply uses uAh. */
...
void oplus_chg_ui_soc_decimal_init(void)
{
	...
	if (!chip || chip->framework_stopping)
		return;
	if (oplus_vooc_get_fastchg_started() == true) {
...
static void oplus_chg_show_ui_soc_decimal(struct work_struct *work)
{
	...
	if (!chip->framework_stopping &&
			chip->calculate_decimal_time <= MAX_UI_DECIMAL_TIME) {
...
static int oplus_chg_awake_init(struct oplus_chg_chip *chip)
{
	if (!chip)
		return -EINVAL;
...
static void oplus_chg_set_awake(struct oplus_chg_chip *chip, bool awake)
{
	...
	if (awake && !chip->suspend_ws_active) {
		chip->suspend_ws_active = true;
		__pm_stay_awake(chip->suspend_ws);
	} else if (!awake && chip->suspend_ws_active) {
		__pm_relax(chip->suspend_ws);
		chip->suspend_ws_active = false;
	}
#endif
}
...
{
	const char *thread_name = "shortc_thread";
	...
	INIT_DELAYED_WORK(&chip->mmi_adapter_in_work,
			  mmi_adapter_in_work_func);
	chip->framework_works_initialized = true;

	rc = oplus_chg_awake_init(chip);
	if (rc < 0)
		goto err;
	chip->framework_awake_initialized = true;

	chip->shortc_thread = kthread_create(shortc_thread_main, chip,
					     thread_name);
	if (IS_ERR(chip->shortc_thread)) {
		rc = PTR_ERR(chip->shortc_thread);
		chip->shortc_thread = NULL;
		chg_err("Can't create shortc_thread: %d\n", rc);
		goto err;
	}
	...
	if (rc) {
		...
		chip->framework_fb_registered = true;
	}
#endif

	rc = oplus_chg_debug_info_init();
	if (rc < 0)
		goto err;
	chip->framework_debug_initialized = true;
	...
	init_ui_soc_decimal_proc(chip);
	init_charger_proc(chip);
	chip->framework_proc_initialized = true;

	schedule_delayed_work(&chip->update_work,
			      OPLUS_CHG_UPDATE_INIT_DELAY);
	charger_xlog_printk(CHG_LOG_CRTI, " end\n");
	return 0;

err:
	oplus_chg_deinit(chip);
...
void oplus_chg_deinit(struct oplus_chg_chip *chip)
{
	...
	was_published = g_charger_chip == chip;
	chip->framework_stopping = true;

#ifdef CONFIG_FB
	if (chip->framework_fb_registered) {
#ifdef CONFIG_DRM_MSM
		msm_drm_unregister_client(&chip->chg_fb_notify);
#else
		fb_unregister_client(&chip->chg_fb_notify);
#endif
		chip->framework_fb_registered = false;
	}
#endif

	if (chip->framework_proc_initialized) {
		remove_proc_subtree("charger_log", NULL);
		remove_proc_subtree("charger_cycle", NULL);
		remove_proc_subtree("charger_critical_log", NULL);
		remove_proc_subtree("tbatt_pwroff", NULL);
		remove_proc_subtree("batt_param_noplug", NULL);
#ifdef CONFIG_OPLUS_RTC_DET_SUPPORT
		remove_proc_subtree("rtc_reset", NULL);
		remove_proc_subtree("vbat_low", NULL);
#endif
		remove_proc_subtree("charging_limit_time", NULL);
		remove_proc_subtree("charging_limit_current", NULL);
		remove_proc_subtree("ui_soc_decimal", NULL);
		remove_proc_subtree("charger", NULL);
		chip->framework_proc_initialized = false;
	}

	if (chip->framework_works_initialized) {
		cancel_delayed_work_sync(&chip->update_work);
		cancel_delayed_work_sync(&chip->ui_soc_decimal_work);
		cancel_delayed_work_sync(&chip->mmi_adapter_in_work);
		chip->framework_works_initialized = false;
	}

	if (chip->shortc_thread) {
		kthread_stop(chip->shortc_thread);
		chip->shortc_thread = NULL;
	}
	if (chip->tbatt_pwroff_task) {
		wake_up_interruptible(&oplus_tbatt_pwroff_wq);
		kthread_stop(chip->tbatt_pwroff_task);
		chip->tbatt_pwroff_task = NULL;
	}

	if (chip->framework_debug_initialized) {
		oplus_chg_debug_info_deinit();
		chip->framework_debug_initialized = false;
	}

	if (was_published)
		oplus_vooc_battery_deinit();

	if (chip->framework_awake_initialized) {
		...
		chip->suspend_ws_active = false;
#endif
		chip->framework_awake_initialized = false;
	}
	...
	chip->pmic_spmi.psy_registered = false;
...
{
	if (READ_ONCE(chip->input_suspend))
		return;
	if (!chip->authenticate) {
		return;
	}
...
int oplus_chg_set_input_suspend(struct oplus_chg_chip *chip, bool suspend)
{
	if (!chip || !chip->chg_ops || !chip->chg_ops->charging_disable)
		return -EINVAL;

	WRITE_ONCE(chip->input_suspend, suspend);
	if (!suspend)
		return 0;

	/* A USB_ICL vote cannot stop the separate VOOC power path. */
	oplus_vooc_stop_fastchg();
	if (chip->charger_exist)
		oplus_chg_turn_off_charging(chip);
	else
		chip->chg_ops->charging_disable();
...
bool oplus_chg_get_input_suspend(void)
{
	struct oplus_chg_chip *chip = READ_ONCE(g_charger_chip);

	return chip && READ_ONCE(chip->input_suspend);
}
/*
...
{
	if (READ_ONCE(chip->input_suspend)) {
		charger_xlog_printk(CHG_LOG_CRTI, " input suspended, return\n");
		return;
	}
	if (oplus_short_c_batt_is_prohibit_chg(chip)) {
...
bool oplus_chg_wake_update_work(void)
{
	...
	if (!g_charger_chip || g_charger_chip->framework_stopping) {
...
void oplus_chg_unsuspend_charger(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true &&
	    !READ_ONCE(g_charger_chip->input_suspend)) {
		g_charger_chip->chg_ops->charger_unsuspend();
	}
}
	...
	while (!kthread_should_stop()) {
		...
		if (!tbatt_pwroff_enable) {
			...
			wait_event_interruptible(oplus_tbatt_pwroff_wq,
				tbatt_pwroff_enable == 1 || kthread_should_stop());
			if (kthread_should_stop())
				break;
		}
		if (oplus_vooc_get_fastchg_started() == true) {
...
int oplus_tbatt_power_off_task_init(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR(chip->tbatt_pwroff_task)) {
		int rc = PTR_ERR(chip->tbatt_pwroff_task);

		chip->tbatt_pwroff_task = NULL;
		chg_err("ERROR: chip->tbatt_pwroff_task create fail: %d\n", rc);
		return rc;
	}
