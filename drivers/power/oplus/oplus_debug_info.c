...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_debug_info.notify_type)
	{
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				...
				chip->mmi_chg, chip->boot_reason, chip->boot_mode, chip->chargerid_volt, chip->chargerid_volt_got);
...
#if 0
static int oplus_chg_get_vooc_max_input_power(void)
{
	int i;
...
static int oplus_chg_debug_info_parse_dt(void)
{
	...
	for(i = 0; i < OPLUS_FASTCHG_OUTPUT_POWER_MAX; i++) {
		...
		g_vooc_charge_strategy_p[i] = kcalloc(count + 1,
				sizeof(struct vooc_charge_strategy), GFP_KERNEL);
		if (!g_vooc_charge_strategy_p[i]) {
			of_node_put(node);
			return -ENOMEM;
		}
...
int oplus_chg_debug_info_init(void)
{
	...
	if (!oplus_chg_debug_info.oplus_chg_debug_wq) {
		rc = -ENOMEM;
		goto err;
	}
	INIT_DELAYED_WORK(&oplus_chg_debug_info.soc_load_dwork, oplus_chg_soc_load_dwork);

	rc = oplus_chg_debug_info_parse_dt();
	if (rc < 0)
		goto err;

#ifdef OPLUS_CHG_DEBUG_TEST
	{
		rc = charge_monitor_test_init();
		if (rc < 0)
			goto err;
	}
#endif
...
void oplus_chg_debug_info_deinit(void)
{
	int i;

	oplus_chg_debug_info.initialized = 0;
	if (oplus_chg_debug_info.oplus_chg_debug_wq) {
		cancel_delayed_work_sync(&oplus_chg_debug_info.soc_load_dwork);
		destroy_workqueue(oplus_chg_debug_info.oplus_chg_debug_wq);
		oplus_chg_debug_info.oplus_chg_debug_wq = NULL;
	}

	if (oplus_chg_debug_info.usb_psy) {
		power_supply_put(oplus_chg_debug_info.usb_psy);
		oplus_chg_debug_info.usb_psy = NULL;
	}
	if (oplus_chg_debug_info.batt_psy) {
		power_supply_put(oplus_chg_debug_info.batt_psy);
		oplus_chg_debug_info.batt_psy = NULL;
	}

	for (i = 0; i < OPLUS_FASTCHG_OUTPUT_POWER_MAX; i++) {
		kfree(g_vooc_charge_strategy_p[i]);
		g_vooc_charge_strategy_p[i] = NULL;
	}

#ifdef OPLUS_CHG_DEBUG_TEST
	remove_proc_subtree("charge_monitor_test", NULL);
#endif
}
