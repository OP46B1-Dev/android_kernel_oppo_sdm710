...
void oplus_vooc_battery_deinit(void)
{
	if (g_vooc_chip && g_vooc_chip->batt_psy) {
		power_supply_put(g_vooc_chip->batt_psy);
		g_vooc_chip->batt_psy = NULL;
	}
}
...
bool oplus_vooc_get_oppo_power_active(void)
{
	return g_vooc_chip &&
	       (READ_ONCE(g_vooc_chip->oppo_power_session) ||
		READ_ONCE(g_vooc_chip->dpdm_switch_mode) == VOOC_CHARGER_MODE);
}
...
static void oplus_vooc_fastchg_func(struct work_struct *work)
{
	...
	bool fastchg_active_before = READ_ONCE(chip->fastchg_started) ||
				     READ_ONCE(chip->fastchg_ing);

	if (oplus_chg_get_input_suspend()) {
...
out:
	/*
	 * NORMAL mode may have been selected above. Re-open DRP only after the
	 * OPLUS D+/D- path has been released.
	 */
	if (!oplus_vooc_get_oppo_power_active())
		oplus_smb_update_otg_policy();
	...
	if (oplus_chg_get_input_suspend())
		oplus_vooc_stop_fastchg();
	if (fastchg_active_before !=
	    (READ_ONCE(chip->fastchg_started) ||
	     READ_ONCE(chip->fastchg_ing)))
		oplus_smb_notify_usb_changed();
...
void oplus_vooc_stop_fastchg(void)
{
	struct oplus_vooc_chip *chip = READ_ONCE(g_vooc_chip);
	bool active;

	if (!chip)
		return;

	active = READ_ONCE(chip->fastchg_started) ||
		 READ_ONCE(chip->fastchg_ing) ||
		 READ_ONCE(chip->fastchg_dummy_started) ||
		 READ_ONCE(chip->fastchg_to_normal) ||
		 READ_ONCE(chip->fastchg_to_warm) ||
		 oplus_vooc_get_oppo_power_active();

	WRITE_ONCE(chip->fastchg_allow, false);
	if (active) {
		del_timer_sync(&chip->watchdog);
		oplus_chg_set_chargerid_switch_val(0);
		WRITE_ONCE(chip->btb_temp_over, false);
		chip->vops->set_switch_mode(chip, NORMAL_CHARGER_MODE);
		oplus_smb_update_otg_policy();
		if (chip->vops->reset_mcu)
			chip->vops->reset_mcu(chip);
	...
	WRITE_ONCE(chip->allow_reading, true);
	WRITE_ONCE(chip->fastchg_started, false);
	WRITE_ONCE(chip->fastchg_ing, false);
	WRITE_ONCE(chip->fastchg_dummy_started, false);
	WRITE_ONCE(chip->fastchg_to_normal, false);
	WRITE_ONCE(chip->fastchg_to_warm, false);
	WRITE_ONCE(chip->fastchg_low_temp_full, false);
