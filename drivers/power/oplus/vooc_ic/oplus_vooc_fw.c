...
void opchg_set_switch_mode(struct oplus_vooc_chip *chip, int mode)
{
	...
	case VOOC_CHARGER_MODE:	       /*11*/
		/*
		 * color-11.1 makes PM660 Sink Only before withdrawing the
		 * DWC3 ID path. Preserve that order here, and publish the final
		 * D+/D- mode only after the sink policy has succeeded.
		 */
		WRITE_ONCE(chip->oppo_power_session, true);
		rc = oplus_smb_update_otg_policy();
		if (rc < 0) {
			WRITE_ONCE(chip->oppo_power_session, false);
			oplus_smb_update_otg_policy();
			chg_err("failed to enter Sink Only before VOOC rc=%d\n",
				rc);
			return;
		}
		opchg_set_switch_fast_charger(chip);
		WRITE_ONCE(chip->dpdm_switch_mode, mode);
	...
	WRITE_ONCE(chip->dpdm_switch_mode, mode);
	/* D+/D- is back on AP before ordinary DRP can be restored. */
	WRITE_ONCE(chip->oppo_power_session, false);
}
