...
static int
fw_get_filesystem_firmware(struct device *device, struct firmware_buf *buf, unsigned int opt_flags)
{
	loff_t size;
	int i, len;
	int rc = -ENOENT;
	...
	/* OPLUS firmware comparison must be serviced by the userspace helper. */
...
{
	char *compare_envp[] = { "FwUp=compare", NULL };
	int retval = 0;
	...
	if (opt_flags & FW_OPT_UEVENT) {
		...
		if (opt_flags & FW_OPT_COMPARE)
			kobject_uevent_env(&fw_priv->dev.kobj, KOBJ_CHANGE,
					   compare_envp);
		else
