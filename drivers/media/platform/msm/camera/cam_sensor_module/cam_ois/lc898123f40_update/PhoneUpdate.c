...
const DOWNLOAD_TBL DTbl[] = {{0x0100, CcMagicCodeF40_01_00,
                              sizeof(CcMagicCodeF40_01_00), CcFromCodeF40_01_00,
                              sizeof(CcFromCodeF40_01_00)},
...
//********************************************************************************
UINT_8 F40_FlashBurstWrite(const UINT_8 *NcDataVal, UINT_32 NcDataLength,
                           UINT_32 ScNvrMan) {
  UINT_32 i, j, UlCnt;
  UINT_8 data[163];
  UINT_32 UlReadVal;
...
//********************************************************************************
void F40_CalcChecksum(const UINT_8 *pData, UINT_32 len, UINT_32 *pSumH,
                      UINT_32 *pSumL) {
  UINT_64 sum = 0;
  UINT_32 dat;
  UINT_16 i;
...
UINT_8 F40_FlashUpdate(UINT_8 flag, DOWNLOAD_TBL *ptr) {
  ...
  CAM_INFO(CAM_OIS, "Fromcode: UlReadVal = 0x%x, UsChkBlocks = %d", UlReadVal,
           UsChkBlocks);

  RamRead32A(0xF00D, &SiWrkVl1);
  CAM_INFO(CAM_OIS, "step test exit: SiWrkVl0=0x%x, SiWrkVl1=0x%x", SiWrkVl0,
           SiWrkVl1);
...
UINT_8 WrChangeCalData_07_0C_DC(UINT_8 uc_index) {
  ...
  if (ans == 0) {
    ...
    if (!uc_index) { // From V07 to VDC
      UlBufDat[GYRO_GAIN_X] =
          (UINT_32)((unsigned long)UlBufDat[GYRO_GAIN_X] / 100 * 87);
      UlBufDat[GYRO_GAIN_X] =
          (UINT_32)((unsigned long)UlBufDat[GYRO_GAIN_X] / 100 * 90);
    ...
    CAM_INFO(CAM_OIS, "after: GYRO_GAIN_X=0x%x, GYRO_GAIN_Y=0x%x",
             UlBufDat[GYRO_GAIN_X], UlBufDat[GYRO_GAIN_Y]);
    ...
    F40_WriteCalData(
        UlBufDat,
        &UiChkSum1); // NVR2Ö²®lðwriteµCheckSumðvZ
    //------------------------------------------------------------------------------------------------
    // Calculate calibration data checksum
    //------------------------------------------------------------------------------------------------
    F40_ReadCalData(
        UlBufDat,
        &UiChkSum2); // NVR2Öwrite³êÄ¢é²®lÌCheckSumðvZ
...
UINT_8 WrGyroGain(UINT_32 ul_gain_x, UINT_32 ul_gain_y) {
  ...
  if (ans == 0) {
    ...
    // Write Hall calibration data
    //------------------------------------------------------------------------------------------------
    F40_WriteCalData(UlBufDat,
                     &UiChkSum1); // NVR2へ調整値をwriteしCheckSumを計算
    //------------------------------------------------------------------------------------------------
    // Calculate calibration data checksum
    //------------------------------------------------------------------------------------------------
    F40_ReadCalData(UlBufDat,
                    &UiChkSum2); // NVR2へwriteされている調整値のCheckSumを計算
